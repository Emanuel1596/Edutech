## Objetivo del documento

Este documento tiene como objetivo llevar el control de las entregas del Sprint 1, indicando qué actividad corresponde a cada integrante, qué debe entregar y cuál es el estado actual de su entrega.

Las actividades de este seguimiento corresponden al primer sprint del proyecto EduTech. :contentReference[oaicite:0]{index=0}

---

## Estados posibles

| Estado | Significado |
|---|---|
| Validado | La persona entregó, explicó y justificó correctamente su actividad |
| Aceptado con correcciones | La entrega sirve, pero necesita ajustes |
| Pendiente | La entrega no está completa o todavía no se ha revisado |
| No validado | La persona no pudo explicar, justificar o defender su entrega |

---

## Seguimiento de entregas

| Integrante | Actividad | Entregable | Estado |
|---|---|---|---|
| Mejia Osornio Valeria | Actores del sistema | Lista de actores y descripción | Validado |
| Huerta Ruiz Diego Rafael | Acciones de los actores | Tabla de actores y acciones | Validado |
| Villanueva Garcia Emanuel | Requerimientos funcionales y no funcionales | Lista de RF y RNF | Validado |
| Romero Palacios Randy | Mapa de navegación | Diagrama o esquema de navegación | Validado |
| Martinez Mejia Edgar | Pantallas del sistema | Lista de pantallas con explicación | Valido |
| Contreras Gonzalez Andre Yahir | Wireframes o bocetos | Bocetos de pantallas principales | Pendiente |
| Villanueva Garcia Emanuel | Modelo de base de datos | Entidades, atributos y relaciones | Validado |
| Mejia Osornio Valeria | Diagrama entidad-relación | Diagrama ER | Aceptado con correciones |
| Romero Palacios Randy | Tecnologías | Lista de tecnologías y uso | Pendiente |
| Martinez Mejia Edgar | Estructura de carpetas | Organización inicial del proyecto | Pendiente |
| Mitad Todos | Frontend visual | Primer avance visual | Pendiente |
| Mitad Todos | Base de datos | Script o estructura inicial de BD | Pendiente |

---

## Observaciones del cierre

- El diagrama entidad-relación se considera aceptado con correcciones porque todavía requiere ajustes en acomodo visual y relaciones.
- El frontend visual cuenta con avance, pero no se considera completamente terminado.
- La base de datos queda pendiente si todavía no existe script SQL o estructura implementada.
- Las actividades pendientes deben retomarse antes de cerrar por completo el Sprint 1.

